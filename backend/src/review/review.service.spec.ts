...
describe("ReviewService", () => {
  it("returns mistakes for the current visitor with distinct mastery status labels and colors", async () => {
  ...
  it("returns bookmarked questions for the current visitor", async () => {
    const prisma = prismaMock({
  ...
  it("returns practice and exam records for the current visitor without creating or scoring exams", async () => {
    ...
    expect(prisma.practiceAttempt.findMany).toHaveBeenCalledWith(
      expect.objectContaining({ where: { visitorId: "v1" }, include: { question: true } })
    );
